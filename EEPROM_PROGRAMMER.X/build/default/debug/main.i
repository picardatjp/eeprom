...
# 53 "main.c"
void shiftOut(unsigned,unsigned,int);
void pulseLow(unsigned);
void pulseHigh(unsigned);
...
int main() {
    ...
    shiftOut(PORTCbits.RC0,PORTCbits.RC3,51);

    return 0;
}

void shiftOut(unsigned dataPin, unsigned clkPin, int data)
{
    int i;
    for(i=0;i<8;i++)
    {
        dataPin = (data & 0b00000001) ? 1 : 0;
        data >> 1;
        pulseLow(PORTCbits.RC3);
        _delay((unsigned long)((1000)*(500000/4000.0)));
    }
}

void pulseHigh(unsigned pin)
{
    pin = 0;
    pin = 1;
    pin = 0;
